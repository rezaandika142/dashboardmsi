...
<body class="h-full bg-gray-100">
    <div class="min-h-screen bg-gradient-to-br from-green-400 via-teal-400 to-blue-400 flex items-center justify-center">
        <div class="bg-gradient-to-br from-white via-gray-100 to-gray-200 shadow-2xl rounded-xl p-8 w-full max-w-md">
            <h1 class="text-3xl font-extrabold text-gray-800 text-center mb-6">Registrasi</h1>
            <form action="{{ route('register') }}" method="POST" class="space-y-6">
                @csrf
                <div class="relative">
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Nama</label>
                    <input type="text" id="name" name="name"
                        class="w-full px-4 py-3 rounded-md bg-gradient-to-br from-gray-100 via-white to-gray-50 shadow-inner focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 transition duration-300 ease-in-out"
            ...
            <div class="text-center mt-4 text-sm text-gray-600">
                Sudah punya akun? <a href="{{ route('login') }}" class="text-teal-500 hover:underline">Login</a>
            </div>
        </div>
    </div>
